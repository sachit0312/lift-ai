...
- **Live Activity** (`src/services/liveActivity.ts`): Persistent iOS Live Activity for entire workout. Unified widget view shows exercise name + set counter always, rest timer controls conditionally. All functions async (return `Promise<void>`), no-op on Android. Deep link: `liftai://workout`. Rest timer end notification shows banner with "Rest Complete" title and plays sound (`timeSensitive` interruption level). `cancelTimerEndNotification` is properly serialized (await before schedule). `updateWorkoutActivityForRest` takes `(exerciseName, totalSeconds, setNumber, totalSets)` — passes set info as subtitle for unified view.
- **Interactive Lock Screen** (`plugins/withInteractiveLiveActivity/`): iOS 17+ interactive buttons. `UnifiedWorkoutView` shows exercise name + set counter always, rest timer (countdown + progress bar + -15s/+15s/Skip controls) conditionally when resting. No Complete Set button — set completion is app-only. Rest state shown conditionally via `timerEndDateInMilliseconds > 0`. Timer `Text` and `ProgressView` use `.id(restEndTime)` to force SwiftUI recreation on +/-15s adjustments. Architecture: RN writes `WidgetState` to App Groups UserDefaults -> Swift App Intents enqueue actions -> RN polls every 500ms via `useWidgetBridge` hook. **Key gotchas**: All 3 intents (DecreaseRest/IncreaseRest/SkipRest) must be zero-parameter structs (parameterized `@Parameter` intents fail silently on Live Activity buttons). Plugin must run AFTER `expo-live-activity` in `app.config.ts`.
- **Workout Bridge** (`src/services/workoutBridge.ts`): RN <-> iOS widget IPC via App Groups UserDefaults. Low-level read/write of `WidgetState` to UserDefaults and action queue polling. All no-op on Android.
- **Hooks** (`src/hooks/`): Extracted from WorkoutScreen for testability. `useRestTimer` — manages rest countdown interval, AppState foreground resync, vibration, and Live Activity updates; exposes `isResting`/`currentEndTime`/`startRestTimer`/`adjustRestTimer`/`dismissRest`; takes `onRestEnd`/`onRestUpdate` callbacks for widget sync. `useWidgetBridge` — builds widget state from exercise blocks (with `preferBlockIdx` + `lastActiveBlockRef` for active exercise tracking), syncs to UserDefaults + Live Activity, handles widget actions (skipRest/adjustRest), manages polling lifecycle; takes callbacks `onDismissRest`/`onAdjustRest`. Exports `LocalSet` and `ExerciseBlock` types used by WorkoutScreen. `useNotesDebounce` — debounced exercise notes persistence.
...
- **WorkoutScreen**: Handles idle + active states inline (no separate file). **Live set modification tracking (F3)**: `ExerciseBlock` has optional `originalWarmupSets`/`originalWorkingSets` stamped at workout start from template data. `computeSetDiffs()` / `hasSetChanges()` in `src/utils/setDiff.ts` compare current tag counts vs originals for F5 "update template?" prompt. Counts restored on resume via template lookup. `ExerciseBlockItem` is a `React.memo` sub-component for per-block re-render isolation. Tags: tap set number to cycle (working -> warmup W -> failure F -> drop D). Long-press or swipe-left to delete set. Rest timer auto-starts on set completion when enabled (per-exercise seconds from template, default 150s). Set completion requires weight + reps (red background validation). Finish requires 1+ completed sets. **Set row layout**: 5 columns (SET, LBS, REPS, RPE, Checkbox) — no dedicated PREV column. Previous data shown as gray input placeholders (`rgba(107, 107, 114, 0.5)`), upcoming targets as purple placeholders (`rgba(124, 92, 252, 0.45)`). Inputs are borderless background pills (surfaceLight on surface). All set changes persist to SQLite immediately. **Auto-fill on completion**: Checking a set auto-fills empty weight/reps from upcoming targets (priority 1) or previous session (priority 2). RPE auto-fills from targets only. User-entered values preserved. **RPE rules**: Warmup sets hide RPE (empty). Failure sets hide RPE (inherently RPE 10, stored as null). **Planned vs Actual**: Starting from upcoming workout persists `upcoming_workout_id` on the workout and writes `target_weight`/`target_reps`/`target_rpe` to each workout set, so comparisons survive workout completion and sync. **Exercise notes** are permanent (persist on both finish and cancel), synced to Supabase via fire-and-forget after debounced save. **Coach tips**: collapsible purple-tinted section per exercise block, only shown when starting from upcoming workout with per-exercise notes from MCP. **Warmup styling**: Uncompleted warmup sets have a subtle amber tint (`colors.warningBg`); completed green overrides. **Last performed badge**: Template cards on idle screen show "Today"/"3 days ago"/etc. subtitle via `getLastPerformedByTemplate()` + `formatLastPerformed()` (`src/utils/formatLastPerformed.ts`). **PR toast**: On set completion, if estimated 1RM beats exercise's all-time best (cached as `bestE1RM` on `ExerciseBlock`), shows "PR" badge on checkbox and fires distinct double-tap haptic. `getBestE1RM()` fetched per-exercise at workout start. **Auto-reorder**: On first set completion for an out-of-position exercise, auto-moves it to the top of incomplete blocks (below fully-completed blocks) with `LayoutAnimation`. Triggered only when `prevCompletedCount === 0` and block is below the first incomplete position. Un-completing does NOT trigger reorder. PR `bestE1RM` update uses exercise ID lookup (not blockIdx) to handle post-reorder index shifts. **Template update prompt (F5)**: On finish, if workout was template-based and user modified set counts (F3) or exercise order (F2), summary screen shows "Template Changes Detected" with change descriptions and "Update Template" button. `buildTemplateUpdatePlan()` / `computeOrderDiff()` in `src/utils/setDiff.ts` compute a `TemplateUpdatePlan` eagerly in `confirmFinish()` while blocks are available. `applyWorkoutChangesToTemplate()` in `database.ts` applies set count + order changes atomically in a single transaction, then fire-and-forget syncs. **Session notes**: Free-form multiline TextInput below "Add Exercise" button. Stored in `workouts.session_notes`. 500ms debounced save via `updateWorkoutSessionNotes()`. Initialized on start/resume, persisted on finish via `finishWorkout()`. Visible to MCP AI coach via `get_workout_history` and `get_workout_detail`. Distinct from exercise-level notes (permanent per-exercise) and coach tips (MCP per-exercise).

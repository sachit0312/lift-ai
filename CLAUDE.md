...
- `syncToSupabase()` — push exercises, templates, workouts + sets to Supabase (auth-guarded, adds user_id). Includes `upcoming_workout_id` on workouts and `target_weight`/`target_reps`/`target_rpe` on workout sets.

...
- Upcoming Workout: Idle screen shows templates immediately; background load first pulls exercises & templates via `pullExercisesAndTemplates()` (5s timeout), reloads templates, then pulls upcoming workout via `pullUpcomingWorkout()` (5s timeout) then `getUpcomingWorkoutForToday()` and animates in when ready. Shows a "Workout Ready" card with exercise count and notes. Starting from upcoming workout pre-populates exercise blocks and shows target values as purple placeholders in the LBS/REPS input fields (using `colors.primaryLight`). When both target and previous data exist, target wins as the placeholder. Template tap shows inline spinner on that card while loading.
...
- **Morning**: Phone app pulls upcoming workout from Supabase, shows target values as purple placeholders in input fields
...
- Sync triggers: on login (AuthContext SIGNED_IN: `clearAllLocalData` → `pullExercisesAndTemplates` → `pullUpcomingWorkout`), on workout finish (`syncToSupabase`), on WorkoutScreen focus (`pullExercisesAndTemplates` → reload templates → `pullUpcomingWorkout`)

## Auth
- `src/contexts/AuthContext.tsx` — AuthProvider with session management via Supabase. Exposes `useAuth()` hook returning `{ session, user, loading }`. On `SIGNED_IN` with new user, clears local data, pulls exercises & templates, then pulls upcoming workout.

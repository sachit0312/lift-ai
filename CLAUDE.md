...
- **ExerciseDetailModal** (`src/components/ExerciseDetailModal.tsx`): Single scrollable page opened by tapping exercise name in any screen. Header (name, type badge, muscle groups, close X) → e1RM banner → Form Notes textarea ("Synced with coach" badge, saves to `form_notes`) → Machine Settings textarea ("Private" badge, saves to `machine_notes`) → Recent History (3 rows) + "See all" → nested ExerciseHistoryModal. Each textarea: local state + 500ms debounce → `updateExerciseFormNotes()` / `updateExerciseMachineNotes()` → fire-and-forget sync → `onExerciseUpdated?()`. Coach notes (`exercise.notes`) NOT rendered. TestIDs: `form-notes-input`, `machine-notes-input`.
- **ExerciseHistoryModal** (`src/components/ExerciseHistoryModal.tsx`): 1RM chart (purple) + volume chart (green). PR banner shows "Current" (freshness-weighted, 42-day half-life decay) alongside "All-time" e1RM when current < all-time. Plateau detection (5+ sessions). Requires 3+ sessions for charts. Uses new two-path e1RM engine. Accessed via "See all" in ExerciseDetailModal.

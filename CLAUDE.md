...
- **Types**: All DB types in src/types/database.ts. `TemplateExercise` and `UpcomingWorkoutSet` include optional `target_rpe` field for coach-prescribed RPE.
...
- `pullExercisesAndTemplates()` — pulls exercises and templates from Supabase into local SQLite (auth-guarded). Uses `INSERT ... ON CONFLICT` to upsert while preserving local-only columns (`exercises.notes`, `template_exercises.rest_seconds`, `template_exercises.target_rpe`). Deletes template_exercises removed remotely (e.g. by MCP). Pulls exercises first (FK dependency), then templates with their template_exercises.

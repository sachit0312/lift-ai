...
- Sync triggers: on login (AuthContext SIGNED_IN: `clearAllLocalData` → `pullExercisesAndTemplates` → `pullWorkoutHistory` → `pullUpcomingWorkout`), on workout finish (`syncToSupabase`), on WorkoutScreen focus (`pullExercisesAndTemplates` → reload templates → `pullWorkoutHistory` → `pullUpcomingWorkout`)

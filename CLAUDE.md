...
- **ExerciseDetailModal** (`src/components/ExerciseDetailModal.tsx`): Single scrollable modal opened by tapping exercise name in any screen. Two tabs: **Details** (e1RM banner, Form Notes with "Synced with coach" badge, Machine Settings with "Private" badge) and **History** (PR banner with current vs all-time, 1RM chart, volume chart, plateau detection, recent sessions with per-set RPE/tag badges). Each note textarea: local state + 500ms debounce → `updateExerciseFormNotes()` / `updateExerciseMachineNotes()` → fire-and-forget sync → `onExerciseUpdated?()`. Coach notes (`exercise.notes`) NOT rendered. TestIDs: `tab-details`, `tab-history`, `form-notes-input`, `machine-notes-input`.
...
**Tools (18)**: get_workout_history, get_workout_detail, get_exercise_list (includes notes + form_notes), search_exercises (includes notes + form_notes), get_all_templates, get_template, get_personal_records, get_exercise_history (returns exercise_notes + exercise_form_notes + sessions), get_upcoming_workout, create_exercise (supports notes, form_notes), update_exercise (name/type/muscle_groups/description/training_goal/notes/form_notes, ownership-checked — never exposes machine_notes), add_exercise_to_template (rest_seconds default 150, warmup_sets default 0), remove_exercise_from_template, create_template, update_template (batch updates: sort_order, default_sets, rest_seconds, warmup_sets), update_template_exercise_rest, reorder_template_exercises (accepts exercise_ids in desired order), create_upcoming_workout (supports per-exercise `notes` for coach tips, `rpe` -> `target_rpe`, `tag` -> SetTag). **MCP access rules**: `notes` (coach) and `form_notes` readable/writable by MCP. `machine_notes` NEVER exposed to MCP (user-private).
...
- `syncToSupabase()` — push exercises (including notes, form_notes, machine_notes), templates, workouts + sets to Supabase (auth-guarded, adds user_id). Includes `upcoming_workout_id` and `session_notes` on workouts and `target_weight`/`target_reps`/`target_rpe` on workout sets.
- `deleteTemplateFromSupabase(id)` / `deleteTemplateExerciseFromSupabase(id)` — fire-and-forget deletes, never throw, errors to Sentry
- `pullExercisesAndTemplates()` — upserts exercises with last-write-wins for notes, form_notes, machine_notes (Supabase value overwrites local). Syncs rest_seconds/warmup_sets bidirectionally. Deletes remotely-removed template_exercises. Batch-fetches with `.in()`.

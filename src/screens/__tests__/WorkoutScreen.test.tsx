...
describe('WorkoutScreen', () => {
  ...
  describe('widget bridge integration', () => {
    ...
    it('adjustWeight action updates weight in exercise block', async () => {
      const result = render(<WorkoutScreen />);
      await startWorkoutWithExercise(result);
      ...
      const pollingCallback = (startPolling as jest.Mock).mock.calls[0]?.[0];
      expect(pollingCallback).toBeDefined();

      (updateWorkoutSet as jest.Mock).mockClear();
      (syncStateToWidget as jest.Mock).mockClear();

      // Simulate adjustWeight action from widget
      await act(async () => {
        pollingCallback([{ type: 'adjustWeight', blockIndex: 0, setIndex: 0, delta: 2.5, ts: Date.now() }]);
      });

      await waitFor(() => {
        expect(updateWorkoutSet).toHaveBeenCalledWith('ws-1', expect.objectContaining({ weight: 102.5 }));
      });
      expect(syncStateToWidget).toHaveBeenCalled();
    });

    it('adjustReps action updates reps in exercise block', async () => {
      const result = render(<WorkoutScreen />);
      await startWorkoutWithExercise(result);

      // Enter initial reps
      await act(async () => {
        fireEvent.changeText(result.getByTestId('reps-0-0'), '8');
      });
      ...
      // Simulate adjustReps action from widget
      await act(async () => {
        pollingCallback([{ type: 'adjustReps', blockIndex: 0, setIndex: 0, delta: -1, ts: Date.now() }]);
      });

      await waitFor(() => {
        expect(updateWorkoutSet).toHaveBeenCalledWith('ws-1', expect.objectContaining({ reps: 7 }));
      });
      expect(syncStateToWidget).toHaveBeenCalled();
    });

    it('adjustRest action calls adjustRestTimer', async () => {
      const result = render(<WorkoutScreen />);
      await startWorkoutWithExercise(result);
      ...
      (adjustRestTimerActivity as jest.Mock).mockClear();

      // Simulate adjustRest action from widget
      await act(async () => {
        pollingCallback([{ type: 'adjustRest', delta: 15, ts: Date.now() }]);
      });

      // adjustRestTimer internally calls adjustRestTimerActivity
      expect(adjustRestTimerActivity).toHaveBeenCalledWith(15);
    });

    it('handles multiple rapid adjustments in single poll batch', async () => {
      const result = render(<WorkoutScreen />);
      await startWorkoutWithExercise(result);
      ...
      // Simulate batch of actions from rapid tapping
      await act(async () => {
        pollingCallback([
          { type: 'adjustWeight', blockIndex: 0, setIndex: 0, delta: 2.5, ts: Date.now() },
          { type: 'adjustWeight', blockIndex: 0, setIndex: 0, delta: 2.5, ts: Date.now() + 1 },
        ]);
      });

      // Should persist both increments — second one builds on first
      await waitFor(() => {
        const calls = (updateWorkoutSet as jest.Mock).mock.calls;
        expect(calls.length).toBe(2);
        expect(calls[0][1]).toEqual(expect.objectContaining({ weight: 102.5 }));
        expect(calls[1][1]).toEqual(expect.objectContaining({ weight: 105 }));
      });
    });

    it('ignores adjustWeight with invalid blockIndex', async () => {
      const result = render(<WorkoutScreen />);
      await startWorkoutWithExercise(result);
      ...
      // Simulate action with out-of-bounds blockIndex
      await act(async () => {
        pollingCallback([{ type: 'adjustWeight', blockIndex: 99, setIndex: 0, delta: 2.5, ts: Date.now() }]);
      });

      // Should not crash or call updateWorkoutSet
